# DevOps Evidence-Backed Capability Graph Design

## Goal

Create a reusable React component for the `github.io` app that visualizes the owner's DevOps capabilities against selected DORA capability areas, using evidence similar to what belongs on a LinkedIn profile: skills, learning, work experience, education, certifications, and projects.

The component should feel like an evolution of the existing `DevOpsCapabilityRadar`: a graph or chart that gives a quick visual capability profile, but with every visible score backed by real evidence.

This is not a full dashboard plan and not a DORA operations-metrics implementation. The initial deliverable should be one reusable visualization component plus its data model. The component can later be placed inside a panel, card, portfolio section, route, or dashboard layout by a parent.
...
DORA's capability catalog includes practices that drive software delivery and operational performance. The component should use selected DORA capabilities as the graph's capability dimensions, not the five DORA delivery metrics as required input data.
...
These dimensions are intentionally selected from the DORA catalog because they map cleanly to public portfolio evidence. Other DORA capabilities such as generative organizational culture, job satisfaction, team experimentation, work in process limits, and well-being are valuable, but they are harder to evidence credibly in a personal public portfolio and should stay out of the first component.
...
```ts
DevOpsCapabilityEvidenceGraph
```

Purpose:

Render a visual capability profile where each DORA-aligned capability has:
...
- optional drilldown data for future interaction.

Recommended visual forms:

- Radar chart for a compact overall profile.
- Matrix or bar chart if radar labels become too dense on mobile.
- Storybook stories should include at least one dense-evidence case and one sparse-evidence case.

The implementation should prefer the charting/styling direction already used by `DevOpsCapabilityRadar` unless a later implementation plan finds a clear reason to change chart type.
...
- `0`: no evidence; do not render this capability.
...
Certification weighting should be high but not absolute. For example, a CNCF certification can strongly support Containers, Reliability, or Security, but it should not automatically make every related capability a `5` without applied evidence.
...
The component should not render dimensions with score `0`. If only three dimensions have evidence, render only those three or use an alternate compact chart that handles sparse data better than a radar.

## Visual Behavior

The first implementation should choose one visual form and keep the component reusable:

- Do not include card or panel chrome inside `DevOpsCapabilityEvidenceGraph`.
- Do not include page headings or route-level layout.
- Keep dimensions stable so the chart can be placed in different containers.
...
- Provide accessible summary text listing each rendered capability and score.

Recommended interactions:

- Basic hover tooltip may show score, evidence count, and strongest evidence.
- Click/drilldown can be deferred. If implemented later, the component should expose selected capability through props/callbacks rather than owning a modal.
...
- Building many separate chart components before the primary graph exists.
...
- Replacing the existing `DevOpsCapabilityRadar` unless the implementation plan chooses to evolve it into this component.

## Recommended Build Order

1. Define DORA capability keys and labels.
2. Define the evidence item model and seed real evidence data.
3. Implement the score derivation utility with tests.
4. Build `DevOpsCapabilityEvidenceGraph` with Storybook examples.
5. Add accessibility summary tests and sparse-data visibility tests.
6. Decide later where to place the component in the `github.io` app.

This order keeps the work aligned with the actual goal: one evidence-backed chart component first, composition later.
...
- accessible text summary for rendered graph values;
