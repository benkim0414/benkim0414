...
Add optional Simple Icons logos to the skill tokens inside DevOps roadmap nodes. The roadmap remains a read-only React Flow diagram with no heading or UI controls. Tokens keep their compact purple Astryx-aligned shape, and logos appear only when a matching Simple Icons asset is available.
...
Create a roadmap-local `DevOpsRoadmapSkillToken` component. It should render a compact purple token with the same visual radius as the token itself and an optional leading icon slot. The token should use Astryx tokens and existing component patterns, but can be a small custom inline element if Astryx `Badge` does not support the required icon/radius composition cleanly.
...
Use an explicit local mapping from roadmap skill label to imported Simple Icons icon. Do not dynamically import arbitrary icon names at runtime. This keeps the bundle predictable and makes unavailable icons intentional.
