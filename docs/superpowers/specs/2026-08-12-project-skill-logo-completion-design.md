...
gains exact official marks for `Yazi`, `mise`, `gh-dash`, and `Herdr`, plus the
approved community-maintained Codex mark published by LobeHub Icons.
...
One explicit exception applies to `Codex`: OpenAI confirms that Codex has an app
icon but does not publish a reusable standalone asset through the official Codex
documentation or open-source Codex repository. The user approved LobeHub Icons'
Codex-specific SVG as a community-maintained representation. Its provenance is
identified accurately rather than describing it as official OpenAI artwork.
This approval does not establish a general fallback to third-party icon packs.
...
## Approved Community Asset

| Skill | Asset | Source | Intended local form |
| --- | --- | --- | --- |
| Codex | Codex color mark | `lobehub/lobe-icons` at `befa2f8022c22985891e5d28aa706f9fba8c578d` | Unmodified compact SVG |

The Codex SVG is MIT-licensed by LobeHub, contains no scripts, event handlers,
embedded remote resources, or `foreignObject` content, and has SHA-256
`4a2f43ce46b5b6e3722c95088f88d26ef91e6a8c2e598e70642a1c54367386e4`.
The provenance record links both LobeHub's Codex page and the pinned repository
revision and states that LobeHub is not an official OpenAI asset source.
...
`ripgrep`, `zoxide`, `delta`, `LazyGit`, or `SSH`. They remain text-only.
...
resolver. It imports the six new assets as URLs and adds exact label mappings
for `Amazon S3`, `Yazi`, `mise`, `gh-dash`, `Herdr`, and `Codex`.
...
- `Amazon S3` resolves to the official service asset rather than the AWS
  wordmark;
- `Yazi`, `mise`, `gh-dash`, `Herdr`, and `Codex` resolve to their vendored
  assets;
- `eza`, `fzf`, `ripgrep`, `zoxide`, `delta`, `LazyGit`, and `SSH` remain
  text-only;
...
- Introducing unofficial, generated, parent-platform, or community substitute
  icons beyond the explicitly approved LobeHub Codex asset.
