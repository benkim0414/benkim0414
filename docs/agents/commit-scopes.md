...
CI checks only commits introduced between an explicit base and head. Pull requests check both introduced commit subjects and the proposed squash title; pushes check the before/after range. A missing, all-zero, or unreachable base fails with an adoption message instead of linting all legacy history. For a new branch or rewritten boundary, record and supply an explicit full baseline OID before rerunning.
