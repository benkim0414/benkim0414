---
title: GitHub.io Astryx Attribution Footer
date: 2026-08-28
...
# GitHub.io Astryx Attribution Footer

## Context

The github.io app needs a compact site footer that says "Built with [Astryx
logo] by @benkim0414", with `@benkim0414` linking to the public GitHub profile.
The Astryx logo itself should link to the Astryx site. The app already uses
Astryx, React Router, a persistent `GlobalNavigationLayout`, and an Astryx
`LayoutContent` scroll owner.
...
`Built with Astryx by @benkim0414` gives enough sentence context for the
...
- Visible text: `Built with [Astryx logo] by @benkim0414`
...
Implement the Astryx mark as a small local `AstryxLogo` component using the
official inline SVG path. Keep `fill="currentColor"` and size it in `em` or a
small fixed token-sized box so it tracks footer typography. Set the SVG color to
the official rendered Astryx logo blue, not the app's theme accent or
`--color-icon-blue`, because this is product attribution rather than a themed
control. Wrap the mark in an Astryx `Link` to `https://astryx.atmeta.com`; use an
accessible label on the link or image because the visible word "Astryx" is
intentionally absent from the footer copy.

Test that the footer renders once, uses the `contentinfo` role through the
semantic footer, includes the Astryx accessible name, links to
`https://astryx.atmeta.com` and `https://github.com/benkim0414`, keeps the
GitHub handle typography aligned with the footer copy, and does not add a
second `LayoutContent`. Run the focused footer, Storybook route, and layout
tests, then build the app and Storybook when the shell structure changes.
