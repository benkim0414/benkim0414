...
describe('devOpsRoadmapItems', () => {
  ...
  it('stores Kubernetes certifications under Container Orchestration', () => {
    const containerOrchestration = devOpsRoadmapItems.find(
      (item) => item.id === 'container-orchestration',
    );

    expect(containerOrchestration).toMatchObject({
    ...
    expect(containerOrchestration?.certifications?.[0]).toBe(
      kubernetesCertifications.cka,
    );
    expect(containerOrchestration?.certifications?.[1]).toBe(
      kubernetesCertifications.ckad,
    );
    expect(containerOrchestration?.certifications?.[2]).toBe(
      kubernetesCertifications.kcna,
    );
  });
});
