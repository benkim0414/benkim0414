...
Astryx Item rows in zero-gap stacks whose compact descriptions state highlight and
relevant-skill counts without end content. The rows activate standalone card
fragments under the router base path, with BrowserRouter coverage for the
arrival scroll and focus behavior.
...
disclose. The evidence summary test protects the Item row, router-base fragment
link, and BrowserRouter navigation to the focused card. The authored stories
include a mobile viewport for the
